...
#include <stdint.h>
#include <string.h>
struct record_t {//An ordinary record
	uint32_t uid;
	uint16_t unused1;
	uint16_t unused2;
	uint32_t start_time;
...
struct pair_t {//A record of UID and maximum session time
	uint32_t uid;
	uint32_t sessionLen;
};
...
void updateUsers(struct pair_t users[2048],size_t* currentCount,struct pair_t user) {
	...
	//At this point, the uid was not found in the users array
...
void printUser(struct pair_t user) {
	char buf[128];
	int n=sprintf(buf,"UID: %i, Longest Session: %i seconds\n",user.uid,user.sessionLen);
	write(1,buf,n);
}
int main(int argc, char* argv[]) {
	if(argc!=2) {
		errx(1,"Wrong number of arguments. Got %d, expected 1",argc-1);
	}
	struct stat st;
	if(stat(argv[1],&st)==-1) {
